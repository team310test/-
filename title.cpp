...
void Title::update()
{
    ...
    case 1:
        ...
        // 演出が終わっていなかったらreturn
        if (isStartPerform_ && !statePerform()) return;
...
bool Title::objFadeIn()
{
    ...
    if (startCommand_->renderer_->color_.w <= 1.0f)
    {
        startCommand_->renderer_->color_.w += fadeSpeed;
        // 超過修正
        if (startCommand_->renderer_->color_.w > 1.0f)
            startCommand_->renderer_->color_.w = 1.0f;
    }
